window.onload = function() {
    fetch('http://localhost:3000/librarians')
        ...
        .then(data => {
                ...
                button4.addEventListener('click', function(){
                    ...
                    add.addEventListener('click', function(){  
                        ...
                        fetch('http://localhost:3000/register', {
                                ...
                                body: JSON.stringify({ name: name, surname: surname, lastname: lastname, email: email, password: password}),
                            })
                            .then(response => {
                                if(!response.ok){
            ...
            // Код, который отрабат. при нажатии на кнопку "Изменить"
                ...
                // Код, который отрабат. при нажатии на кнопку "Удалить"
                button.addEventListener('click', function() {
                    const id = event.target.parentElement.parentElement.children[0].textContent;
                    fetch(`http://localhost:3000/deletelibr/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                        },
